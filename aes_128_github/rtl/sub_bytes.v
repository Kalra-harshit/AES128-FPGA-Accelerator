...
module sub_bytes (
    input  wire [127:0] state_in,   // NOW accepts full 128 bits
    output wire [127:0] state_out   // NOW outputs full 128 bits
);

    // We define the S-Box as a function so we can reuse it 16 times
    ...
    // Now we apply the S-Box to ALL 16 bytes of the 128-bit input
    genvar i;
    generate
        for (i = 0; i < 16; i = i + 1) begin : sbox_loop
            // This magic loop grabs 8 bits at a time and transforms them
            assign state_out[i*8 +: 8] = sbox_lookup(state_in[i*8 +: 8]);
        end
    endgenerate
